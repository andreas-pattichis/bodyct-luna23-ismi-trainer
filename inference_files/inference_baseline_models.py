...
def perform_inference_on_test_set(workspace: Path):
    ...
    # load the parameter values for each of the architectures from the folders where this parameters are stored
    ckpt = torch.load(
        workspace / "results/20240526_0_segmentation/fold0/best_model.pth"
    )
    segmentation_model.load_state_dict(ckpt)

    # ⚠️ make sure to adjust these paths
    ckpt = torch.load(workspace / "results/20240524_0_malignancy_BALANCED_DATASET/fold0/best_model.pth")
    ...
    for idx, image_path in enumerate(tqdm(list(test_set_path.glob("*.mha")))):
        ...
        with torch.no_grad():
            outputs = {
                ...
                "noduletype": noduletype_model(image)["noduletype"],
                "malignancy": malignancy_model(image)["malignancy"],
            }
