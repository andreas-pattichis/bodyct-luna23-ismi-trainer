...
class NoduleAnalyzer:
    ...
    def forward(self, batch_data, update_weights=False):
        ...
        images = images[:,images.shape[1]//2:images.shape[1]//2+1,:,:]
        masks = masks[:,masks.shape[1]//2:images.shape[1]//2+1,:,:]
        ...
        if "noduletype" in self.tasks:
            ...
            outputs = (
                outputs.data.cpu().numpy().reshape(-1, 4)
            )
    ...
    def train(self, model):
        ...
        # Freezing all the parameters all the network at this point
        for param in model.parameters():
            param.requires_grad = False

        # This code allows the updating of the parameters of the last layer (which is the only one that is trained)
        #the other layer's parameters remain frozen because the ViT is a pretrained model.
        for param in getattr(model, last_layer_name).parameters():
        ...
        for epoch in range(self.max_epochs):
            ...
            for mode in ["training", "validation"]:
                ...
                for batch_data in tqdm(data):
                    ...
                    for task in self.tasks:
                        predictions[task].extend(outputs)
...
if __name__ == "__main__":
    ...
    def best_metric_fn(metrics):
        return metrics["noduletype"]["balanced_accuracy"]  # 🥚 Easter egg
    
    #now we load the model extraction model ViT which is pretrained from timm library
    #we add a last classification layer which takes the output of the ViT model and connects it
    #with as many neurons as number of classes for the noduletype task (to perform the classification)

    pretrained_model = timm.create_model('vit_base_patch16_224', pretrained=True)
    num_classes = 4 # Example number of classes
    classification_layer = nn.Linear(in_features=768, out_features=num_classes)

    # Replace the classifier with your custom classification layer
    pretrained_model.head = classification_layer
    ...
    nodule_analyzer.train(pretrained_model)  # 🥚 Easter egg
